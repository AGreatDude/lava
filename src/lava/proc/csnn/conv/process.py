...
class CSNNConvolution(AbstractProcess):
    ...
    ----------
    weights : np.ndarray
        Convolution weight kernel of shape `(filter_width, filter_height,
        input_depth, output_filters)`.
    thresholds : np.ndarray or float or int
        Threshold values for each output feature map.
    input_shape : tuple of int
        Input shape as `(width, height, channels)`.
    stride : int or tuple of int, optional
        Stride of the convolution. Default is 1.
    padding : int or tuple of int, optional
        Zero-padding added to both sides of the input. Default is 0.
    wta_infer : bool, optional
        If True, uses winner-take-all inference per spatial position.
        Default is False.
    name : str, optional
        Name of the process.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the CSNNConvolution process."""
